...
public class UsingReflection {
	public static void main(String[] args) {
		...
		try {
			...
			for(Method me : mes) {
				if(me.getModifiers()>4000) {continue;}//4000초과면 다음스탭 반복문 빠져나옴
				...
				System.out.printf("(","");//2게 이상이면 , 로 표시
				Class[] aa =me.getParameterTypes(); //아규먼트(인자,파라미터)
				for(int j=0;j<aa.length;j++) {
					System.out.printf("%s",toRe(aa[j].getName()));//아규먼트이름
